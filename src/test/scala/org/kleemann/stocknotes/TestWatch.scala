...
class TestWatch extends munit.FunSuite {
    ...
    test("one buy") {
        assertEquals(
            ...
            Right(BuyWatch(Some(Currency.dollarsCents(20, 0)), None, Fraction.one))
        )
        assertEquals(
            Watch.parse("BUY 100.00", Fraction.one),
            Right(BuyWatch(Some(Currency.dollarsCents(100, 0)), None, Fraction.one))
        )
    }
    ...
    test("two buy") {
        assertEquals(
            ...
            Right(BuyWatch(Some(Currency.dollarsCents(40, 0)),Some(Currency.dollarsCents(20, 0)), Fraction.one))
        )
    }
    ...
    test("one sell") {
        assertEquals(
            ...
            Right(SellWatch(None, Some(Currency.dollarsCents(9, 99)), Fraction.one))
        )
    }
    ...
    test("two sell") {
        assertEquals(
            ...
            Right(SellWatch(Some(Currency.dollarsCents(40, 0)),Some(Currency.dollarsCents(20, 0)), Fraction.one))
        )
    }
    ...
    test("multiple") {
        val w = SellWatch(Some(Currency.dollarsCents(40, 0)),Some(Currency.dollarsCents(20, 0)), Fraction.one)
        val two = Fraction(2,1)
        // doubling the mult is a 2:1 split which should half the price
        assertEquals( w.lowAtMult(two), Some(Currency.dollarsCents(20,0)))
        assertEquals(w.highAtMult(two), Some(Currency.dollarsCents(10,0)))
    }

    test("complex multiple") {
        // test multiple changes over time
        val m1 = Fraction.one
        val w1 = BuyWatch(Some(Currency.dollarsCents(10, 0)), None, m1)
        assertEquals(w1.lowAtMult(m1), Some(Currency.dollarsCents(10, 0)))
        ...
        assertEquals(w1.lowAtMult(m2), Some(Currency.dollarsCents(2, 0)))
        // 2:1 split, shares double, prices are cut in half
        val m3 = m2*Fraction(2,1)
        assertEquals(w1.lowAtMult(m3), Some(Currency.dollarsCents(1, 0)))
        // a reverse 1:3 split, every share becomes 3, price multiplies by three
        val m4 = m3*Fraction(1,3)
        assertEquals(w1.lowAtMult(m4), Some(Currency.dollarsCents(3, 0)))
    }
}
