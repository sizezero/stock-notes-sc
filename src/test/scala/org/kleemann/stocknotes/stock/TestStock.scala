...
class TestStock extends munit.FunSuite {
    ...
    test("empty file") {
        ...
        val required = Stock(
            ...
            None,
            Set(),
            List(Entry(ticker, Date.earliest, Nil)),
            List[Trade](),
            BuyWatch.none,
            SellWatch.none
        )
    ...
    test("simple single entry") {
        ...
        val obtained = Stock.load(ticker, "filename", g)
        val entries = List(
            Entry(ticker, Date.earliest, List("""
                |
                |""".stripMargin)),
            Entry(ticker, Date(1992,1,1).get, List("foobar\n"))
        )
        val required = Stock(
            ticker,
            Some("Microsoft Corp"),
    ...
    test("bad date order") {
        ...
        val obtained = Stock.load(ticker, "filename", g)
        assertEquals(obtained, Left("filename(4): date January 1, 1980 is not greater than previous date January 1, 1992"))
    }
